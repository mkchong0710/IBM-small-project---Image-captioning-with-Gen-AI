...
gen_parms = {
    ...
    "temperature": 0.7    # Specifying the temperature which controls the randomness of the token generated
...
def generate_cover_letter(company_name, position_name, job_description, resume_content):
    ...
    prompt = f"Generate a customized a cover letter using the company name: {company_name}, the position applied for: {position_name}, and the job description: {job_description}. Ensure the cover letter highlights my qualifications and experience as detailed in the resume content: {resume_content}. Adapt the content carefully to avoid including experiences not present in my resume but mentioned in the job description. The goal is to emphasize the alignment between my existing skills and the requirements of the role."
...
cover_letter_app = gr.Interface(
    ...
    inputs=[
        ...
        gr.Textbox(label="Job Description Information", placeholder="Paste the job decription here...", lines=10),
